...
public class PlayersController {
   ...
   @GetMapping({"/", "/players"})
    ...
    public String showEdit(@PathVariable Long id,Model model) {
        Player player = player = this.playerService.findById(id);
     ...
     */
    @PostMapping({"/players"})
    public String create(@RequestParam String name, @RequestParam String bio, @RequestParam Double pointsPerGame,@RequestParam PlayerPosition position,@RequestParam Long team, Model model) {
        Player player = this.playerService.create(name, bio, pointsPerGame, position, team);
    ...
    public String update(@PathVariable Long id,@RequestParam String name,@RequestParam String bio,@RequestParam Double pointsPerGame, @RequestParam PlayerPosition position,@RequestParam Long team, Model model) {
        Player player = this.playerService.update(id, name, bio, pointsPerGame, position, team);
        List<Player> players = playerService.listAllPlayers();
        model.addAttribute("players", players);
        model.addAttribute("position", PlayerPosition.values());
        model.addAttribute("team", teamService.listAll());
    ...
    public String vote(@PathVariable Long id, Model model) {
        Player player = this.playerService.vote(id);
        List<Player> players = playerService.listAllPlayers();
        model.addAttribute("players", players);
        model.addAttribute("position", PlayerPosition.values());
        model.addAttribute("team", teamService.listAll());
